...
import PhoneNumberInput, { isValidPhoneNumber } from 'react-phone-number-input/mobile';
...
const CustomInput = forwardRef<HTMLInputElement, InputComponentProps>(({ className, ...props }, ref) => (
  <input
    {...props}
    ref={ref}
    className={`${className || ''} custom-phone-input focus:outline-none text-lg placeholder:text-black`}
  />
));
...
const ContactForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await axios.post('http://localhost:3000/contact', jsonData, {
        headers: {
    ...
    <div className="h-full flex flex-col items-center justify-center bg-gray-300 px-4 " >
      <div className="bg-white shadow-lg flex md:flex-row flex-col gap-10 rounded-lg p-8 w-full max-w-5xl mx-4 mt-20">
        <div className="md:w-1/2 flex flex-col justify-center">
          <img src="https://plus.unsplash.com/premium_photo-1677529496297-fd0174d65941?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" alt="About Us" className="rounded-lg mb-4 w-full" />
          <div className="text-lg text-gray-700 mb-4 max-sm:hidden">
            <h1 className="text-4xl font-bold mb-4">About Us</h1>
            <p className="para">
              We are committed to providing exceptional service and value to our customers. Our team of professionals is dedicated to ensuring your satisfaction with every interaction. Whether you have questions, need support, or simply want to learn more about what we offer, we are here to help. Reach out to us today, and let's start a conversation.
            </p>
          </div>
        ...
        <form id="ContactForm" className="md:w-1/2 space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <PhoneNumberInput
              ...
              className="w-full h-12 p-2 border-gray-300 bg-transparent border rounded-md placeholder-gray-500 focus:ring-blue-500 focus:border-blue-500"
            />
            {value && isValidPhoneNumber(value) ? '' : <p className='text-red-600 text-sm'>Invalid Phone Number</p>}
          </div>
          ...
          <div className="">
            <button type="submit" disabled={isLoading} className='w-full py-3 mt-8 bg-blue-500 hover:bg-blue-700 text-white rounded-lg'>
              {isLoading ? 'Sending...' : 'Submit'}
            </button>
          </div>

          {successMessage && <span className="block text-green-500">{successMessage}</span>}
          {errorMessage && <span className="block text-red-500">{errorMessage}</span>}
        </form>
      </div>
    </div>
  );
